...
Now we add columns that multiply the PROPDMG and PROPDMGEXP columns and the CROPDMG and CROPDMGEXP columns respectively to get dollar amounts for each event.  
```{r}
#replacing PROPDMGEXP variables with numbers according to their power 
#replacing 'K' or 'k' with 1000
a <- grep("K|k", pDataEcon$PROPDMGEXP, ignore.case=TRUE)
pDataEcon[a,6] <- "1000"

#replacing 'M' or 'm' with 1000000
a <- grep("M|m", pDataEcon$PROPDMGEXP, ignore.case=TRUE)
pDataEcon[a,6] <- "1000000"

#replacing 'B' or 'b' with 1000000000
a <- grep("B|b", pDataEcon$PROPDMGEXP, ignore.case=TRUE)
pDataEcon[a,6] <- "1000000000"
...
#replacing CROPDMGEXP variables with numbers according to their power 
#replacing 'K' or 'k' with 1000
...
```{r}
plot <- qplot(Pop$EVTYPE, Pop$Count, color = Pop$Casualty, stat="identity") + geom_bar() + labs(title = 
    "Casualties for Major Weather Events in the U.S., 1950 - 2011", x = "Event Type", y = "# Casualties") + labs(color = "Type of Casualty")
print(plot)
```
